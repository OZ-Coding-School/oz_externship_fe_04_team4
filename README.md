...
- 서비스 URL: [StudyHub*Admin*바로가기](https://admin.ozcoding.site)
...
- 운영자가 반복 작업(조회/필터/정렬/검수)을 빠르게 처리할 수 있도록
  회원, 탈퇴, 스터디, 리뷰, 구인 공고 등 주요 관리 대상을 하나의 시스템으로 통합

- 권한(Role) 기반으로 접근/기능 제한을 명확히 분리하여
  관리자·스태프·일반 유저 간 역할에 맞는 안전한 운영 환경 제공

- **인증/토큰 흐름 표준화로 운영 안정성 확보(401 자동 처리 포함)**를 통해
  인증 오류 및 세션 만료 상황에서도 일관된 사용자 경험과 시스템 안정성 유지

- 회원 가입/탈퇴 추세, 탈퇴 사유 분포 등 시각화된 데이터 제공으로
  운영자의 직관적 상황 파악 및 데이터 기반 의사결정 지원

**기대효과**

1. 운영 업무 최적화

- 회원 및 스터디 관리 기능을 시스템화하여 반복적인 수작업 리소스를 절감
- 검색, 필터, 정렬 기능을 통해 대규모 데이터도 빠르고 정확하게 처리 가능
- 관리자 페이지 내 즉시 수정·삭제·복구가 가능해 운영 대응 속도 향상

2. 서비스 신뢰도 향상

- 부적합 콘텐츠(리뷰, 구인 공고 등)를 신속하게 관리·삭제하여 플랫폼 품질 유지
- 탈퇴 요청 회원 복구 기능을 통해 사용자 이슈에 유연하고 빠른 대응 가능
- 권한 기반 기능 제어로 오남용 및 보안 리스크 최소화

3. 객관적 성과 지표 확보

- 회원 가입·탈퇴 추세, 탈퇴 사유 분포를 그래프와 차트로 시각화
- 단순 추측이 아닌 데이터에 기반한 서비스 성장 분석 가능
- 운영·기획·개선 방향에 대한 합리적인 의사결정 근거 제공
